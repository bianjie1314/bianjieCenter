...
public class PhoneInfoController extends BaseController {
    ...
    public String qryPhoneList(@RequestBody PageRequest pageParam){
        logger.info("list -- param:{}",JSONObject.toJSONString(pageParam));
        JSONArray jsonArray = new JSONArray();
        try{
            Map paramContent = pageParam.getParam();
            paramContent.put("currentPage",pageParam.getCurrentPage());
            paramContent.put("pageSize",pageParam.getPageSize()>30?15:pageParam.getPageSize());
            ...
            if (count > 0){
                ...
                for (PhoneInfo info:phoneInfoList){
                    ...
                    itemJson.put("saleMoney", Double.parseDouble(info.getSaleMoney())/100);
                    itemJson.put("purchaseMoney", Double.parseDouble(info.getPurchaseMoney())/100);
                    itemJson.put("minConsumer", Double.parseDouble(info.getMinConsumer())/100);
